...
FASTAPI_EMBEDDING_URL = os.environ.get("FASTAPI_EMBEDDING_URL", "http://localhost:8000/embedding_AMS")                                                                                                   
...
def process_face_enrollment_background(employee_id: str, img_b64: str):
    ...
    try:
        ...
        logger.info(f"[ENROLL] Step 1 OK — Decoded image. shape={frame_rgb.shape}")
        ...
        logger.info(f"[ENROLL] Step 2 OK — Detected {len(bboxes)} face(s)")
        ...
        if not bbox or len(bbox) != 4:
            ...
            logger.warning(f"[ENROLL] ABORT — Invalid bbox format: {bbox}")
        ...
        logger.info(f"[ENROLL] Step 3 OK — BBox: x1={x1}, y1={y1}, x2={x2}, y2={y2}")
        ...
        logger.info(f"[ENROLL] Step 4 OK — Face crop shape={face_bgr.shape}")
        ...
        logger.info(f"[ENROLL] Step 5 OK — Generated {len(aug_imgs)} augmented images")
        ...
        for i, img_bgr in enumerate(aug_imgs):
            ...
            if emb.shape[0] != 512:
                ...
                logger.warning(f"[ENROLL] Embedding {i} has wrong shape: {emb.shape[0]}")
        ...
        logger.info(f"[ENROLL] Step 6 OK — Got {len(embs)}/{len(aug_imgs)} valid embeddings")
        ...
        logger.info(f"[ENROLL] Step 6 filter — Kept {len(keep_idx)}/{len(embs)} embeddings (thr={thr:.3f})")
        ...
        logger.info(f"[ENROLL] Step 7 OK — Prototype embedding shape={proto.shape}, norm={np.linalg.norm(proto):.4f}")
        ...
        try:
            ...
            logger.info(f"[ENROLL] Step 8 — Pushing to {FASTAPI_EMBEDDING_URL}")
            ...
            logger.info(f"[ENROLL] Step 8 — Response: status={r.status_code}, body={r.text[:200]}")
